...
namespace SoftCob.Views.ListaTrabajo
{
    ...
    public partial class WFrm_NuevaListaTrabajo : System.Web.UI.Page
    {
        #region Variables
        ...
        private void FunCargarCombos(int opcion)
        {
            switch (opcion)
            {
                ...
                case 2:
                    ...
                    DdlCampania.DataSource = new ConsultaDatosDAO().FunConsultaDatos(119, int.Parse(ViewState["CodCatalogo"].ToString()), 0, 0, "",
                        "", "", ViewState["Conectar"].ToString());
        ...
        private bool FunValidarCampos()
        {
            ...
            {
                new FuncionesBAS().FunShowJSMessage("Seleccione Estrategia..!", this);
                _validar = false;
            }
        ...
        protected void ImgPreview_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                else new FuncionesBAS().FunShowJSMessage("Seleccione Datos para preview / Campos o Datos en Grid..!", this);
            }
            catch (Exception ex)
            {
                Lblerror.Text = ex.ToString();
            ...
            try
            {
                ...
                if (ViewState["Nuevo"].ToString() == "0")
                {
                    _dts = new ConsultaDatosDAO().FunConsultaDatos(94, int.Parse(ViewState["CodigoCedente"].ToString()), int.Parse(ViewState["CodCatalogo"].ToString()), 0, "",
                        TxtLista.Text.Trim().ToUpper(), TxtFechaInicio.Text.Trim(), ViewState["Conectar"].ToString());
