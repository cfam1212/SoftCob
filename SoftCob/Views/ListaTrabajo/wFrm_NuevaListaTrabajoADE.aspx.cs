    ...
    public partial class wFrm_NuevaListaTrabajoADE : Page
    {
        ...
        private void FunCargarCombos(int opcion)
        {
            switch (opcion)
            {
                ...
                case 1:
                    ...
                    DdlGestores.DataSource = new CatalogosDTO().FunGetConsultasCatalogo(12, "--Seleccione Gestor--", int.Parse(DdlCedente.SelectedValue), 0, 0, "", "", "", ViewState["Conectar"].ToString());
                    ...
                    break;
                case 2:
                    DdlAccion.DataSource = new SpeechDTO().FunGetArbolAccion(int.Parse(ViewState["CodigoCPCE"].ToString()));
        ...
        private bool FunValidarCampos()
        {
            ...
            if (int.Parse(ViewState["CodigoLista"].ToString()) == 0)
            {
                ...
                {
                    new FuncionesBAS().FunShowJSMessage("Elija Gestor..!", this);
                    _validar = false; ;
                }

                if (ChkGestion.Checked == false && ChkArbol.Checked == false && ChkFecha.Checked == false)
                {
                    new FuncionesBAS().FunShowJSMessage("Elija de Opción Gestión (Gestión - Árbol)", this);
                    _validar = false; ;
                }
                ...
                if (ChkGestion.Checked)
                {
                    ...
                    {
                        new FuncionesBAS().FunShowJSMessage("Seleccione Tipo Gestión..!", this);
                        _validar = false; ;
                    }
                }
                ...
                if (ChkArbol.Checked)
                {
                    ...
                    {
                        new FuncionesBAS().FunShowJSMessage("Seleccione Opción de Árbol..!", this);
                        _validar = false; ;
                    }
                }
                ...
                if (ChkFecha.Checked)
                {
                    if (!new FuncionesBAS().IsDate(txtFechaDesde.Text))
                    {
                        new FuncionesBAS().FunShowJSMessage("Formato Fecha Incorrecta..!", this);
                        _validar = false;
                    }
                    if (!new FuncionesBAS().IsDate(txtFechaHasta.Text))
                    {
                        new FuncionesBAS().FunShowJSMessage("Formato Fecha Incorrecta..!", this);
                        _validar = false;
                    }
        ...
        protected void ImgPreview_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (_continuar)
                {
                    ...
                    {
                        _sql1 = "select distinct gete_cldecodigo from GSBPO_GESTION_TELEFONICA GTE (nolock) ";
                        ...
                        _sql2 += "from GSBPO_GESTION_TELEFONICA GTE (nolock) where gete_cedecodigo=" + DdlCedente.SelectedValue;
                        ...
                        else new FuncionesBAS().FunShowJSMessage("No Existen Registros..!", this);
            ...
            try
            {
                ...
                if (_continuar)
                {
                    ...
                    if (int.Parse(LblTotal.InnerText) > 0)
                    {
                        ...
                        //sql1 = "select CodigoCLDE = CDE.CLDE_CODIGO,FechaGestion = '' from GSBPO_CUENTA_DEUDOR CDE (nolock) ";
                        //sql1 += "INNER JOIN GSBPO_CLIENTE_DEUDOR CLI (nolock)ON CDE.CLDE_CODIGO=CLI.CLDE_CODIGO ";
                        ...
                        //sql2 += "from GSBPO_GESTION_TELEFONICA GTE (nolock) ";
                        ...
                        _mensaje = new EstrategiaDAO().FunCrearListaTrabajo(_codlistaarbol, TxtLista.Text.Trim().ToUpper(),
                            ...
                            (DataTable)ViewState["Estrategia"], "sp_NewListaTrabajo", ViewState["Conectar"].ToString());
