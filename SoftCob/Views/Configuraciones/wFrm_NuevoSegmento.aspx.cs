namespace SoftCob.Views.Configuraciones
{
    ...
    public partial class WFrm_NuevoSegmento : Page
    {
        ...
        protected void ImgAddSegmento_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(TxtSegmento.Text.Trim()))
                {
                    new FuncionesDAO().FunShowJSMessage("Ingrese Nombre del Segmento..!", this, "W", "C");
                    return;
                }

                if (string.IsNullOrEmpty(TxtValorInicial.Text.Trim()) || TxtValorInicial.Text.Trim() == "0")
                {
                    new FuncionesDAO().FunShowJSMessage("Ingrese Valor Segmento Inicial..!", this, "W", "C");
                    return;
                }

                if (string.IsNullOrEmpty(TxtValorFinal.Text.Trim()) || TxtValorFinal.Text.Trim() == "0")
                {
                    new FuncionesDAO().FunShowJSMessage("Ingrese Valor Segmento Final..!", this, "W", "C");
                    return;
                }
                ...
                _filagre["Segmento"] = TxtSegmento.Text.Trim().ToUpper();
        ...
        protected void ImgModiSegmento_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                if (string.IsNullOrEmpty(TxtSegmento.Text.Trim()))
                ...
                _result["Segmento"] = TxtSegmento.Text.Trim().ToUpper();
                _result["Descripcion"] = TxtDescripcion.Text.Trim().ToUpper();
        ...
        protected void ImgSelecc_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                _dtbsegmento = (DataTable)ViewState["SegmentoCabecera"];
                _result = _dtbsegmento.Select("Codigo='" + ViewState["Codigo"].ToString() + "'").FirstOrDefault();
                TxtSegmento.Text = _result["Segmento"].ToString();
                ...
                ImgAddSegmento.Enabled = false;
                ImgModiSegmento.Enabled = true;
            }
            catch (Exception ex)
            {
                Lblerror.Text = ex.ToString();
        ...
        protected void ImgEliminar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                new CedenteDAO().FunDelSegmento(int.Parse(ViewState["CodigoCEDE"].ToString()), int.Parse(ViewState["CodigoCPCE"].ToString()),
                    int.Parse(ViewState["Codigo"].ToString()), 0);
            }
            catch (Exception ex)
            {
                Lblerror.Text = ex.ToString();
            ...
            try
            {
                ...
                foreach (DataRow _dr in _dtbsegmento.Rows)
                {
                    _codigo = new CedenteDAO().FunGetCodigoSegmento(int.Parse(ViewState["CodigoCEDE"].ToString()),
                        int.Parse(ViewState["CodigoCPCE"].ToString()), int.Parse(_dr[0].ToString()), 0);
